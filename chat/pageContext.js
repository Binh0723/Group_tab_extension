...
export async function resolveTargets(pinned) {
    if (pinned.length > 0)
        return pinned.slice(0, MAX_TARGETS);
...
function describeHttpError(status) {
    ...
    if (status === 402)
        return "Firecrawl credits exhausted";
    if (status === 429)
        return "Firecrawl rate limited — try again shortly";
    return `Firecrawl error ${status}`;
}
async function scrapeOne(settings, target) {
    ...
    try {
        ...
        if (!res.ok)
            return { target, error: describeHttpError(res.status) };
        const markdown = extractMarkdown(await res.json());
        if (!markdown || !markdown.trim())
            return { target, error: "no content returned" };
        return { target, markdown: markdown.trim() };
    }
    catch (err) {
        const aborted = err?.name === "AbortError";
        return { target, error: aborted ? "timed out" : err?.message || "request failed" };
    }
}
/** Scrape each target sequentially; identical URLs share one scrape. */
export async function fetchPageContents(targets, settings) {
    const results = [];
    const memo = new Map();
...
export function listTargetMetadata(targets) {
