...
async function groupTabs() {
    ...
    for (let i = 0; i < tabs.length; i += BATCH_SIZE) {
        ...
        for (const g of modelGroups) {
            if (!g || typeof g.name !== "string" || !Array.isArray(g.tabIds))
                continue;
            const name = g.name.trim().slice(0, 24);
            const color = typeof g.color === "string" && COLORS.includes(g.color)
                ? g.color
                : null;
            if (!groupsByName.has(name))
                groupsByName.set(name, { name, color, tabIds: [] });
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
    if (typeof msg !== "object" || msg === null || msg.type !== "GROUP_TABS") {
        return false;
    }
    groupTabs()
