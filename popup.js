...
const actionStatusEl = $("action-status");
...
function chatRegistrableDomain(hostname) {
    const labels = hostname.toLowerCase().replace(/^www\./, "").split(".");
    if (labels.length <= 2)
        return labels.join(".");
    const lastTwo = labels.slice(-2).join(".");
    if (CHAT_TWO_PART_TLDS.has(lastTwo))
...
function renderContextList(tabs, currentId) {
...
chatContextSearch.addEventListener("input", () => {
    ...
    const filtered = q
        ? allContextTabs.filter((t) => t.title.toLowerCase().includes(q) || t.domain.toLowerCase().includes(q))
        : allContextTabs;
...
document.addEventListener("click", (e) => {
    if (!contextMenuOpen)
        return;
    if (chatContextMenu.contains(e.target) || chatContextBtn.contains(e.target))
        return;
    closeContextMenu();
});
